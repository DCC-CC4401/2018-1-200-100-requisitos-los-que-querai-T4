...
from reservationsApp.models import ArticleReservation
...
def article_data(request, article_id):
    try:
        ...
        last_loans = ArticleLoan.objects.filter(article=article, ending_date_time__lt=datetime.now(tz=pytz.utc)).order_by('-ending_date_time')[:10]
        ...
        if request.user.enabled:
            try:
                ...
                if start_date_time > end_date_time:
                    ...
                    reservation = ArticleReservation(article=article, starting_date_time=start_date_time, ending_date_time=end_date_time, user=request.user)
                    reservation.save()
